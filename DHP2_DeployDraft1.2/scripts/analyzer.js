(function () {
  document.addEventListener('DOMContentLoaded', () => {
    ...
    function processLeagueData(rosters, users, tradedPicks, leagueInfo, radarSlots = []) {
      ...
      const globalTotals = [];
      const ppgByPosition = { QB: [], RB: [], WR: [], TE: [] };
      ...
      const teams = (Array.isArray(rosters) ? rosters : []).map((roster) => {
        ...
        const allPlayers = (roster.players || [])
          .map((playerId) => {
            ...
            if (pos && ppgByPosition[pos]) {
              ppgByPosition[pos].push({ playerId, ppg });
            }
            return {
              id: playerId,
              pos,
      ...
      const ppgScores = buildPpgScoreMap(ppgByPosition);

      teams.forEach((team) => {
        team.radarAssignments = assignRadarSlots(team.allPlayers, slotSequence, ppgScores);
      });
    ...
    function assignRadarSlots(players = [], slotSequence = [], ppgScores = {}) {
      ...
      slotSequence.forEach((slot, idx) => {
        ...
        const score = selected
          ? ppgScores[selected.id] ?? (Number(selected.ppg) || 0)
          : 0;
    ...
    function buildPpgScoreMap(ppgByPosition) {
      const scores = {};
      if (!ppgByPosition) return scores;

      Object.entries(ppgByPosition).forEach(([pos, entries]) => {
        const filtered = entries
          .filter((entry) => entry && Number.isFinite(entry.ppg) && entry.ppg > 0)
          .sort((a, b) => b.ppg - a.ppg);

        if (!filtered.length) return;

        const denominator = filtered.length > 1 ? filtered.length - 1 : 1;

        filtered.forEach((entry, index) => {
          const percentile = filtered.length === 1 ? 1 : 1 - index / denominator;
    ...
    function renderRadarChart(teams, radarSlots = state.radarSlots) {
      ...
      const leagueAverages = slots.map((slot, index) => {
        ...
        const average = teams.length ? total / teams.length : 0;
        return Math.min(100, Math.max(0, average));
      });

      const labelColors = userData.map((value) => getPercentileColor(value));
      const maxValue = Math.max(0, ...userData, ...leagueAverages);
      const scaleMax = Math.max(100, Math.ceil(maxValue / 10) * 10);
        ...
        data: {
          ...
          datasets: [
            ...
            },
            {
              ...
              analyzerLabels: true,
              labelColors,
              labelFormatter: (value) => formatPercentile(value),
              order: 2,
            },
          ],
        },
    ...
    function formatPercentile(value) {
      ...
      return `${ordinal(clamped)}%`;
    }

    function getPercentileColor(value) {
      if (!Number.isFinite(value)) return '#bcd2ff';
      if (value >= 90) return '#00ffaf';
      if (value >= 80) return '#58a7ff';
      return '#bcd2ff';
    }
