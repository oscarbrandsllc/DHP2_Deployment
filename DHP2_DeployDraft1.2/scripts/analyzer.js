(function () {
  document.addEventListener('DOMContentLoaded', () => {
      ...
      return {
        ...
        scales: {
          x: {
            ...
            ticks: {
              color: '#EAEBF0',
              font: { size: valueFontSize, family: "'Product Sans', 'Google Sans', sans-serif" },
          ...
          y: {
            ...
            ticks: {
              ...
              font: { size: tickFontSize, weight: '600', family: "'Product Sans', 'Google Sans', sans-serif" },
              callback: (value, index) => {
                const labelIndex = typeof index === 'number' ? index : teams.findIndex((team) => team.teamName === value);
                const teamName = labelIndex >= 0 ? teams[labelIndex]?.teamName : value;
                const resolvedName = typeof teamName === 'string' && teamName.length
                  ? teamName
                  : String(teamName ?? value ?? '');
                if (!compact) return resolvedName;
                return truncateLabel(resolvedName, 11);
              },
            },
          },
        },
        plugins: {
          legend: {
            ...
            labels: {
              color: '#EAEBF0',
              font: { size: compact ? 10 : 12, family: "'Product Sans', 'Google Sans', sans-serif" },
              usePointStyle: true,
            },
          },
          tooltip: {
          ...
          analyzerBarTotals: {
            ...
            font: compact
              ? '600 9px "Product Sans", "Google Sans", sans-serif'
              : '600 11px "Product Sans", "Google Sans", sans-serif',
          },
        },
      };
    }
    ...
    function renderRadarChart(teams, radarSlots = state.radarSlots) {
      ...
      const paddedScaleMax = maxValue > 0 ? scaleMax + scaleStep : scaleMax;
    ...
    function isCompactViewport(maxWidth = MOBILE_BREAKPOINT) {
    ...
    function truncateLabel(label, maxLength) {
      ...
      if (!Number.isFinite(maxLength) || maxLength <= 0) return label;
