(function () {
  document.addEventListener('DOMContentLoaded', () => {
    ...
    function abbreviateName(name) {
      ...
      const first = parts[0];
      const normalizedFirst = first.replace(/[^A-Za-z]/g, '');
      const isAcronym = normalizedFirst.length > 0 && normalizedFirst.length <= 3 && normalizedFirst === normalizedFirst.toUpperCase();
      if (isAcronym) {
        return trimmed;
      }

      const initial = normalizedFirst.charAt(0);
      if (!initial) return trimmed;

      const remainder = parts.slice(1).join(' ');
      return `${initial.toUpperCase()}. ${remainder}`.trim();
    }
    ...
    function renderSummaryStats(teams) {
      ...
      const rankingValue = overallRank ? ordinal(overallRank) : '—';
      const rankingMeta = userTeam.record
      ...
      const topScorerMeta = topScorer?.total
        ? [
            ...
            .join(' • ')
        : 'No scoring data';
      const topScorerValue = topScorer?.shortName || (topScorer?.name ? abbreviateName(topScorer.name) : '—');
      const topScorerTitle = topScorer?.name || '';
      ...
      elements.summaryStats.innerHTML = chips
        .map((chip) => {
          ...
          const meta = escapeHtml(chip.meta || '');
          const accent = chip.accent ? ` style="color: ${chip.accent};"` : '';
          const titleAttr = chip.title ? ` title="${escapeAttribute(chip.title)}"` : '';
          return `
          <article class="analyzer-chip${chip.className ? ` ${chip.className}` : ''}">
            <span class="chip-label">${label}</span>
            <span class="chip-value"${accent}${titleAttr}>${value}</span>
    ...
    function renderLeagueLeaders() {
      ...
      elements.leaderboardBody.innerHTML = leaders
        .map((entry, index) => {
          const displayName = entry.shortName || abbreviateName(entry.name) || entry.name || 'Unknown';
          const nameTitle = entry.name ? ` title="${escapeAttribute(entry.name)}"` : '';
          const owner = entry.owner || 'Unrostered';
          const nflTeam = entry.nflTeam || '--';
          return `
          <tr>
            <td class="col-rank">${index + 1}</td>
            <td class="col-player"${nameTitle}>${escapeHtml(displayName)}</td>
            <td class="col-nfl">${escapeHtml(nflTeam)}</td>
            <td class="col-owner">${escapeHtml(owner)}</td>
            <td class="col-total">${escapeHtml(entry.total.toFixed(1))}</td>
            <td class="col-ppg">${escapeHtml(entry.ppg.toFixed(1))}</td>
          </tr>
        `;
        })
        .join('');
