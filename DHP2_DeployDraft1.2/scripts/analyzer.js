  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    function buildLineupOptions(max, metric, teams) {
      ...
      return {
        ...
        layout: {
          padding: { top: 12, right: 16, bottom: 16, left: 8 },
        },
        scales: {
          x: {
            stacked: true,
          ...
          y: {
            ...
            ticks: {
              ...
              padding: 6,
              font: { size: 10 },
              callback: (value) => truncateLabel(value, 11),
            },
          },
        },
        plugins: {
    ...
    function renderRadarChart(teams, radarSlots = state.radarSlots) {
      ...
      const labelColors = slots.map((slot, index) =>
        getRadarLabelColor(userData[index], leagueAverages[index]),
      );
      const rawMax = Math.max(0, ...userData, ...leagueAverages);
      const scaleMax = rawMax > 0 ? roundUpTo(rawMax * 1.12, 5) : 10;
    ...
    function getRadarLabelColor(value, average) {
      ...
      if (!Number.isFinite(average)) return '#bcd2ff';
      if (value === 0 && average === 0) return '#bcd2ff';
      return value > average ? '#00ffaf' : '#bcd2ff';
    }
