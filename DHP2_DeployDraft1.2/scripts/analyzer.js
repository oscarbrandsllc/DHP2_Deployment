(function () {
  document.addEventListener('DOMContentLoaded', () => {
    ...
    function buildLineupOptions(max, metric, teams) {
      ...
      const tickPadding = isMobile ? 2 : 8;
      const totalsFont = isMobile
            ...
            ticks: {
              ...
              callback(value) {
                const label = typeof this.getLabelForValue === 'function'
                  ...
                  : teams?.[Number(value)]?.teamName;
                return formatTeamLabel(label);
              },
            },
          },
        },
    ...
    function renderRadarChart(teams, radarSlots = state.radarSlots) {
      ...
      const baseMax = maxValue > 0 ? roundUpTo(maxValue, scaleStep) : scaleStep;
      const paddedMax = baseMax > 0 ? roundUpTo(baseMax * 1.08, scaleStep) : baseMax;
      const scaleMax = paddedMax || scaleStep * 2;
    ...
    function formatTeamLabel(label, maxLength = 11) {
      if (!label) return '';
