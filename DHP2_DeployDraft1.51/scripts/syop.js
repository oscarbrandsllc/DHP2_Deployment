(function () {
  ...
  function renderDraftPositional() {
    ...
    const legendContainer = document.getElementById('draft-positional-legend');
    if (legendContainer) {
      legendContainer.innerHTML = '';
      DRAFT_SERIES.forEach((series) => {
        legendContainer.appendChild(createEl('span', { class: 'legend-item' },
    ...
    const margin = width < 520
      ? { top: 32, right: 18, bottom: 48, left: 46 }
      : { top: 32, right: 28, bottom: 56, left: 60 };
    const chartWidth = width - margin.left - margin.right;
    const chartHeight = height - margin.top - margin.bottom;
    const svg = createSVG('svg', {
      viewBox: `0 0 ${width} ${height}`,
    ...
    DRAFT_POSITIONAL.forEach((row) => {
      ...
      entries.forEach(({ series, value }) => {
        const chip = createEl('div', {
          ...
          style: {
            borderColor: hexToRgba(series.color, 0.32),
