  ...
  function renderDraftPositional() {
    ...
    rounds.forEach((round, index) => {
      ...
      sortedSeries.forEach((entry) => {
        ...
        if (isCompact) {
          chipChildren.push(createEl('span', {
          ...
          chipChildren.push(createEl('span', { class: 'draft-round-chip-pos' }, entry.key));
          chipChildren.push(createEl('span', {
            class: 'draft-round-chip-value',
            style: { color: entry.color }
          }, `${entry.value}%`));
        ...
        const chip = createEl('div', {
          class: `draft-round-chip${isCompact ? ' draft-round-chip--compact' : ''}`,
          style: {
            '--chip-accent': entry.color,
            borderColor: hexToRgba(entry.color, isCompact ? 0.35 : 0.55)
          }
        }, ...chipChildren);
