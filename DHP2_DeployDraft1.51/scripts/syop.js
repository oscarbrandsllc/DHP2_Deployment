(function () {
  ...
  function renderDraftPositional() {
    ...
    const chipGap = isCompact ? 5 : 6;
    const chipStackHeight = 4 * chipHeight + 3 * chipGap;
    const baseBottomMargin = width < 540 ? 48 : 56;
    const margin = width < 540
      ? { top: 52, right: 20, bottom: baseBottomMargin + chipStackHeight + 24, left: 54 }
      : { top: 52, right: 28, bottom: baseBottomMargin + chipStackHeight + 28, left: 68 };
    const chartWidth = width - margin.left - margin.right;
    const chartHeight = height - margin.top - margin.bottom;
    const svg = createSVG('svg', {
      viewBox: `0 0 ${width} ${height}`,
    ...
    const axisLabelY = chartHeight + (isCompact ? 24 : 26);
    ...
    DRAFT_SERIES.forEach((series) => {
      ...
      points.forEach((point) => {
        ...
        roundStacks[point.roundIndex].push({
          series,
          value: point.value
        });
      });
    ...
    const chipStartY = axisLabelY + chipHeight / 2 + (isCompact ? 10 : 12);

    roundStacks.forEach((entries, roundIndex) => {
      if (!entries.length) return;

      const x = roundIndex * stepX;
      const sorted = entries.slice().sort((a, b) => b.value - a.value);

      sorted.forEach((entry, idx) => {
        const valueDisplay = Number.isFinite(entry.value)
          ? (Number.isInteger(entry.value) ? entry.value : entry.value.toFixed(1))
          : '—';
        const chipText = `${entry.series.key} ${valueDisplay}%`;
        const textWidth = chipText.length * chipFontSize * 0.56 + 8;
        const rectWidth = textWidth + 2 * chipPadding.x;
        const y = chipStartY + idx * (chipHeight + chipGap);
        ...
        chipGroup.appendChild(createSVG('rect', {
          x: -rectWidth / 2,
          y: -chipHeight / 2,
          width: rectWidth,
          height: chipHeight,
        ...
        chipGroup.appendChild(createSVG('text', {
          x: 0,
          y: 0,
          ...
          'stroke-width': '2.5',
          'stroke-linecap': 'round'
        }, document.createTextNode(chipText)));
